...
def singletask_train(args):
    model_dir = './checkpoint/ner_{}_{}'.format(args.data, args.model_name)
    ...
    with open('./data/{}/{}_predict.pkl'.format(args.data, args.model_name), 'wb') as f:
...
def multitask_train(args):
    ...
    model_dir = './checkpoint/ner_{}_{}'.format('_'.join(args.data.split(',')), args.model_name)
    if args.clear_model:
        clear_model(model_dir)
    data_dir = './data'
    ...
    for data in data_list:
        ...
        with open('./data/{}/{}_{}_predict.pkl'.format(data, args.model_name,
